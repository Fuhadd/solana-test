...
class _ExampleAppState extends State<ExampleApp> {
  ...
  // notable swamp table vibrant father crumble gossip pill congress search broom sister
  ...
  void _signLargeTransactions(final int count) async {
    ...
    try {
      ...
      final List<Transaction> transactions =
          await _createLargeTransactions(connection, count: count);

      setState(() => _status = "$description...");
      final SignTransactionsResult result = await adapter.signTransactions(
        transactions
            .map((transaction) => adapter.encodeTransaction(transaction))
            .toList(),
      );
  ...
  void _signMessages(final int count) async {
    ...
    try {
      ...
      final List<String> messages = List.generate(
          count, (index) => adapter.encodeMessage('Sign message $index'));
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ElevatedButton(
              onPressed:
                  adapter.isAuthorized ? () => _signTransactions(1) : null,
              child: const Text('Sign Transactions (1)'),
            ),
            ElevatedButton(
              onPressed:
                  adapter.isAuthorized ? () => _signTransactions(3) : null,
              child: const Text('Sign Transactions (3)'),
            ),
            ElevatedButton(
              onPressed: adapter.isAuthorized
                  ? () => _signLargeTransactions(10)
                  : null,
              child: const Text('Sign Large Transactions'),
            ),
            ElevatedButton(
              onPressed: adapter.isAuthorized
                  ? () => _signAndSendTransactions(1)
                  : null,
              child: const Text('Sign and Send Transactions (1)'),
            ),
            ElevatedButton(
              onPressed: adapter.isAuthorized
                  ? () => _signAndSendTransactions(3)
                  : null,
              child: const Text('Sign and Send Transactions (3)'),
            ),
            ElevatedButton(
